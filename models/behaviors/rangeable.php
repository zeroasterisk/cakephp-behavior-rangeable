<?php
/**
 * A CakePHP behavior to facilitate a simple reteival of results 
 * via a range search on a lon/lat fields
 *
 ...
 * @author dereuromark <https://github.com/dereuromark>
...
class RangeableBehavior extends ModelBehavior {
	...
	public $milesToDegrees = .01445;
	public $mapMethods = array('/^_findRange$/' => '_findRange');
	public $debug = array(); // false or array()
	...
	*/
	public function _findRange(Model $Model, $method, $state, $query, $results = array()) {
		if ($state == 'before') {
	...
	*/
	public function getRangeQuery(Model $Model, $query, $range, $lat, $lon) {
	...
	*/
	public function getRangeBox($range, $lat, $lon) {
	...
	*/
	public function calculatePrecisionDistance($sourceLat, $sourceLon, $targetLat, $targetLon, $precision = 2) {
